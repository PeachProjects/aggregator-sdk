 ...
 * THENA DEX Tests
 *
 * Test Peach Aggregator's quote and contract functionality for THENA.
 *
 * Test Cases:
 * 1. getQuote with provider=THENA only, verify quote is returned
...
const PROVIDER_NAME = 'THENA';

describe('THENA DEX Tests', () => {
  ...
  // ============================================
  // Test 1: getQuote with provider=THENA only
  // ============================================
  describe('1. getQuote Tests (THENA only)', () => {
    it('should return valid quote for WBNB -> USDT', async () => {
      ...
      expect(quote.params.steps[0].adapter).toBe(PEACH_CONTRACTS.THENA_ADAPTER);
      expect(quote.params.deadline).toBeGreaterThan(0n);

      const protocols = quote.route.routes[0].steps.map(s => s.pool.protocol);
      expect(protocols).toContain(ProtocolType.Thena);
    });
