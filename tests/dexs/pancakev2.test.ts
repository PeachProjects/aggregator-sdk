 ...
 * PancakeV2 DEX Tests
 *
 * Test Peach Aggregator's quote and contract functionality for PancakeV2.
 *
 * Test Cases:
 * 1. getQuote with provider=PANCAKEV2 only, verify quote is returned
...
import { PEACH_API, TEST_CONFIG, PEACH_CONTRACTS, stringify, WBNB, USDT, USDC, BTCB, NATIVE_TOKEN_ADDRESS } from '../helpers';

const PROVIDER_NAME = 'PANCAKEV2';

describe('PancakeV2 DEX Tests', () => {
  ...
  // ============================================
  // Test 1: getQuote with provider=PANCAKEV2 only
  // ============================================
  describe('1. getQuote Tests (PANCAKEV2 only)', () => {
    it('should return valid quote for WBNB -> USDT', async () => {
      ...
      expect(quote.params.steps[0].adapter).toBe(PEACH_CONTRACTS.PANCAKEV2_ADAPTER);
      expect(quote.params.deadline).toBeGreaterThan(0n);

      const protocols = quote.route.routes[0].steps.map(s => s.pool.protocol);
      expect(protocols).toContain(ProtocolType.PancakeV2);
    });
