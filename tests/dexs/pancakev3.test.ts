 ...
 * PancakeV3 DEX Tests
 *
 * Test Peach Aggregator's quote and contract functionality for PancakeV3.
 *
 * Test Cases:
 * 1. getQuote with provider=PANCAKEV3 only, verify quote is returned
...
const PROVIDER_NAME = 'PANCAKEV3';

describe('PancakeV3 DEX Tests', () => {
  ...
  // ============================================
  // Test 1: getQuote with provider=PANCAKEV3 only
  // ============================================
  describe('1. getQuote Tests (PANCAKEV3 only)', () => {
    it('should return valid quote for WBNB -> USDT', async () => {
      ...
      expect(quote.params.steps[0].adapter).toBe(PEACH_CONTRACTS.PANCAKEV3_ADAPTER);
      expect(quote.params.deadline).toBeGreaterThan(0n);

      const protocols = quote.route.routes[0].steps.map(s => s.pool.protocol);
      expect(protocols).toContain(ProtocolType.PancakeV3);
    });
